  ...
  var resizeHandlerPolyfill = function(target, force) {
    ...
    if (target.length) {
      ...
      var dragresize = new DragResize('resize-polyfill', {
        handles: ['br'],
        minWidth: parseInt(computedStyle.minWidth) || textarea.getBoundingClientRect().width,
        minHeight: parseInt(computedStyle.minHeight) || textarea.getBoundingClientRect().height,
        allowBlur: false
      });
